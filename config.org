...
#+export_file_name: Literate Emacs
# #+setupfile: ~/Notes/denote/latex-setups/setup.org
#+setupfile: ~/Notes/denote/latex-setups/latex-setup.org
#+latex_compiler: xelatex
#+options: toc:t num:t
#+LATEX_SRC_BLOCK_BACKEND: listings
#+STARTUP: show2levels
...
*** Substitute (simple text substitution)
#+begin_src emacs-lisp :tangle bard-emacs-modules/bard-emacs-essentials.el :mkdirp yes
...
*** Epub reader (~nov~)
I use this package in tangent with [[https://github.com/nobiot/org-remark][org-remark]] to take notes on books as I am reading them.
#+begin_src emacs-lisp :tangle bard-emacs-modules/bard-emacs-media.el :mkdirp yes
  (use-package nov
    :ensure t
    :after org-remark
...
*** Region face override
This face override makes the region only focus on the text instead of extending all the way to the border of the window. This is more like highlighting in a modern application, which is why I liked it. The emphasis remains on what is highlighted versus the entire area of the highlight.
#+begin_src emacs-lisp :tangle bard-emacs-modules/bard-emacs-theme.el :mkdirp yes
(custom-set-faces
      '(region ((t :extend nil))))
#+end_src
...
*** Basic window split configurations
#+begin_src emacs-lisp :tangle bard-emacs-modules/bard-emacs-window.el :mkdirp yes
  (setq focus-follows-mouse t)
...
*** Replace burying buffers/quitting windows with killing the buffer
I find it super annoying that pressing q removes the buffer from view but doesn't actually kill it. Remapping these functions will make any keybind that uses these functions kill the buffer instead.
...
#+end_src
*** Org mode annotations in other modes (~org-remark~)
#+begin_src emacs-lisp :tangle bard-emacs-modules/bard-emacs-writing.el :mkdirp yes
...
** bard-embark
Main embark configuration found in ~bard-emacs-modules/bard-emacs-embark.el~. This is another one of the files I borrowed from Protesilaos, but renamed/modified so I can remember.
...
Somewhat related to my Elfeed/EMMS Youtube Code.
...
The main configuration for fontaine is found at ~bard-emacs-theme~ - Fonts. These are the presets for different fonts that I can switch between using ~C-c f~.
...
*** Typst lecture notes with denote file naming scheme
#+begin_src emacs-lisp :tangle bard-elisp/bard-writing.el :mkdirp yes
